...
status=$?
if (grep -a "[×❌]" "$(dirname "$JUNIT")/h2spec.out" | grep -aqE 'Sends a GOAWAY frame|Sends multiple WINDOW_UPDATE frames increasing the flow control window to above 2^31-1 on a stream') \
&& ! (grep -a "[×❌]" "$(dirname "$JUNIT")/h2spec.out" | grep -avqE 'Sends a GOAWAY frame|Sends multiple WINDOW_UPDATE frames increasing the flow control window to above 2^31-1 on a stream'); then
