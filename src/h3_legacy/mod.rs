...
pub use crate::h3::quinn;
pub use crate::h3::{qpack, transport};
pub use crate::h3::{Frame, FrameDecoder, FrameError, H3Error, Settings, TransportError};
...
use crate::{h3_legacy::date::DateCache, EarlyHints, HttpProtocol, Incoming, Upgrade, Upgraded};
...
#[inline]
pub(super) fn h3_stream_error_to_io(error: h3::error::StreamError) -> std::io::Error {
    // There could be logic to construct different error kinds,
    // but for now we just convert to other, because for private enum variants.
    std::io::Error::other(error)
}
